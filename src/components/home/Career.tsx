...
const Career = () => {
  ...
  return (
    <div
      ref={divRef} 
      className={`
        max-w-7xl w-full mx-auto mt-40 grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 grid-cols-1 gap-4 
      `} 
    >
      {careerList.map((item, index) => (
        <div key={index} className={`transition-all duration-1000 transform delay ${item.delay} ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-20'}`}>
