...
const RightFullImage :React.FC<RightFullImageProps> = ({bgColor, txtColor, descColor, title, desc, img}) => {
    return (
        <div className={`lg:w-[1240px] w-full mx-auto  h-screen flex flex-col lg:flex-row relative justify-end items-center mt-40 `}>
            
            <div className={`lg:w-1/2 w-full h-full  flex justify-center items-center flex-col  `}>
                <div className={`w-full lg:h-7/12 h-full  flex justify-center lg:items-start items-center  lg:text-left text-center flex-col lg:pl-24 pl-0`}
                  style={{ backgroundColor : bgColor}}>
                    <div className={`lg:text-5xl/12 text-4xl/10 font-bold whitespace-pre-line `}  style={{ color: txtColor }}> {title} </div>
