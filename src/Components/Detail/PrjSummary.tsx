...
const PrjSummary :React.FC<prjSummaryProps>  = ({ desc1, desc2, txtColor}) => {
    return (
            <div className='bg-yellow-00 lg:max-w-5xl sm:max-w-2xl h-full flex sm:flex-row flex-col text-left mx-auto mb-20'>
                <div className='bg-purple-00 sm:w-1/2 w-2/3  mx-auto min-w-[320px] flex flex-col px-0 '>
                    ...
                    <div className='flex lg:flex-row flex-col lg:space-x-5 lg:space-y-0 space-x-0 space-y-1 text-zinc-600 font-semibold mt-2'>
                        <div className='text-sm/6 ' > 인원 <span className=' ml-2' style={{ color: txtColor }}> {desc1[0]}</span></div>
                        <div className='text-sm/6  '> 역할  <span className='ml-2'
                        style={{ color: txtColor }} >{desc1[1]} </span></div>
                        <div className='text-sm/6 '> 기간  <span className='ml-2' style={{ color: txtColor }}>{desc1[2]}</span></div>
                    </div>
